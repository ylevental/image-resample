...
train_dataset = np.loadtxt('mnist_trn_img.txt')
train_dataset = np.reshape(train_dataset, (train_dataset.shape[0], 3, 32, 32))
train_dataset = np.float32(train_dataset)
train_dataset = torch.from_numpy(train_dataset)
train_set = torch.clone(train_dataset)

pl.seed_everything(42)
#train_set, val_set = torch.utils.data.random_split(train_dataset, [45000, 5000])

val_set = np.loadtxt('mnist_tst_img.txt')
val_set = np.reshape(val_set, (val_set.shape[0], 3, 32, 32))
val_set = np.float32(val_set)
val_set = torch.from_numpy(val_set)

# Loading the test set
test_set = torch.clone(val_set)
...
def G_SM1(X, y):
    ...
    #X_resampled, y_resampled = KMeansSMOTE(kmeans_estimator=MiniBatchKMeans(n_init='auto'), cluster_balance_threshold=0.008).fit_resample(X, y)
    ...
    #X_resampled, y_resampled = SOUP().fit_resample(np.copy(X), np.copy(y))
    
    oversampler = sv.MulticlassOversampling(oversampler='SMOTE_IPF')
...
input_imgs = np.loadtxt('mnist_trn_img.txt')
...
dec_y = np.loadtxt('mnist_trn_lab.txt') 
...
np.savetxt('smotemnist_trn_img.txt', resx, fmt='%1.18f')
np.savetxt('smotemnist_trn_lab.txt', resy, fmt='%i') 
